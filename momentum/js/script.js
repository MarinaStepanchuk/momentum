alert('Уважаемый проверяющий, дождитесь пожалуйста полной загрузки страницы и воспользуйся для загрузки режимом инкогнито,чтобы не перетащить свои настройки на проект. Обратите, пожалуйста, внимание, что Unsplash API имеет ограничение в 50 картинок в час (считаются даже перезагрузки страницы), поэтому не злоупотребляйте именно на этом API переключением тегов и картинок, иначе он просто перестанет работать и будет доступен только через час, на деле даже 50 картинок не проигрывает и если пощелкать пару раз подряд он блокируется (об этом скажет ошибка в консоли), но через час снова будет работать. Вот ссылка на документацию,чтобы не быть голословной https://unsplash.com/documentation. Бесконечно можно щелкать на Flickr API, если очень хочется)) Спасибо за внимание')
...
async function getLinkToImageFlickr() {
    const url = `https://www.flickr.com/services/rest/?method=flickr.photos.search&api_key=7898e3e8a50cd9510845c16f05a592ce&tags=${tegBg}&extras=url_l&format=json&nojsoncallback=1`;
    const res = await fetch(url);
    const data = await res.json();
    const img = new Image();
